  ...
  return (
    <div className='mt-12'>
      <div className='grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 x2k:grid-cols-5 x4k:grid-cols-6'>
        {images.map((image, index) => {
          if (images.length === index + 1) {
            ...
            return (
              <div className='mx-auto mb-10' key={image.id}>
                <Card image={image} />
              </div>
            )
          }
      ...
      {!loading && images.length === 0 && (
        <p className='max-w-2xl text-lg font-semibold mx-auto my-2 py-2 px-2 bg-blue-400 text-center rounded'>
          No image result query...
        </p>
      )}
      {error && (
        <p className='max-w-2xl text-lg font-semibold mx-auto my-2 py-2 px-2 bg-red-700 text-center rounded'>
          No Image Result...
        </p>
      )}
      {loading && (
